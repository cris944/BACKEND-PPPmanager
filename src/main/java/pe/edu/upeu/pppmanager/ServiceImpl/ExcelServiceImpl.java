...
public class ExcelServiceImpl implements ExcelService {
    ...
    public ImportResults importData(MultipartFile file) throws IOException, SQLException {
        ...
        try (Workbook workbook = new XSSFWorkbook(file.getInputStream());
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                int idPersona = insertarPersona(conn, apellido, nombre, dni, correo, religion, telefono, pais, "A");
                ...
                int idUsuario = obtenerIdUsuario(conn, idPersona);


                if (!tieneRol(conn, idUsuario, 3)) {
                 skippedRows++;
                 logger.warn("Fila {}: el usuario no tiene el rol 3 asignado. La fila será omitida.", i);
                 result.addErrorMessage("Fila " + i + ": el usuario no tiene el rol 3 asignado.");
                 continue; 
                }


                int idEstudiante = insertarEstudiante(conn, idPersona, ciclo, grupo, correo_institucional, codigo, "A");
    ...
    private void insertarUsuario(Connection conn, int idPersona, String nombre_usuario, String passwordEncrypted, String img_perfil, String estado) throws SQLException {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(insertUsuarioSQL, new String[]{"ID_USUARIO"})) {
...
//fumada :v
    ...
    private void asignarRol(Connection conn, int idUsuario, int rol ) throws SQLException {
        String insertUsuarioRolSQL = "INSERT INTO USUARIO_ROL (ID_USUARIO, ID_ROL, ESTADO) VALUES (?, ?, 'A')";
        try (PreparedStatement pstmt = conn.prepareStatement(insertUsuarioRolSQL)) {
    ...
    private boolean tieneRol(Connection conn, int idUsuario, int rol) throws SQLException {
        String query = "SELECT 1 FROM USUARIO_ROL WHERE ID_USUARIO = ? AND ID_ROL = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(query)) {
            pstmt.setInt(1, idUsuario);
            pstmt.setInt(2, rol);
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    private int obtenerIdUsuario(Connection conn, int idPersona) throws SQLException {
        String query = "SELECT ID_USUARIO FROM USUARIO WHERE ID_PERSONA = ?";
        try (PreparedStatement pstmt = conn.prepareStatement(query)) {
            pstmt.setInt(1, idPersona);
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
        ...
        throw new SQLException("No se encontró un usuario con el ID_PERSONA: " + idPersona);
    }
}
